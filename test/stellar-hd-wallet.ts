...
const MNEMONIC_KOREAN =
  '한쪽 공개 학점 거액 재빨리 주민 해군 조절 종로 여론 성당 송아지';
...
describe('StellarHDWallet', () => {
  describe('fromMnemonic', () => {
    ...
    const expectInvalidMnemonicFailure = (mnemonic?: string) => {
      try {
        StellarHDWallet.fromMnemonic(mnemonic as string);
    ...
    it('empty mnemonic throws', () => {
      ...
      expectInvalidMnemonicFailure('');
      expectInvalidMnemonicFailure(null as unknown as string);
    });
  ...
  describe('validateMnemonic', () => {
    ...
    it('rejects empty mnemonic input', () => {
      ...
      assert.equal(val(null as unknown as string), false);
      assert.equal(val(''), false);
      assert.equal(val('', 'korean'), false);
      assert.equal(val(null as unknown as string, 'korean'), false);
    });
